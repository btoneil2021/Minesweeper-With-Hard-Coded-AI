...
class PatternDetector:
    ...
    def same_bombs_as_squares(self, key):
        ...
        """
        if str(key) not in self.analyzer.copyDict:
            return None

        tile_value = self.analyzer.copyDict[str(key)]
        ...
        if open_square_number == tile_value:
            ...
            for neighbor in self.analyzer.get_neighbors(key):
                if str(neighbor) not in self.analyzer.copyDict:
                    continue
                if self.analyzer.copyDict[str(neighbor)] == AI_UNKNOWN:
    ...
    def all_bombs_found(self, key):
        ...
        for neighbor in self.analyzer.get_neighbors(key):
            if str(neighbor) not in self.analyzer.copyDict:
                continue
            if self.analyzer.copyDict[str(neighbor)] == AI_FLAGGED:
                flags_around += 1
    ...
    def transitive_bomb_property(self, key):
        ...
        for neighbor in self.analyzer.get_cardinal_neighbors(key):
            ...
            elif self.analyzer.copyDict[str(neighbor)] >= -1:
                check_availability -= 1
        ...
        for neighbor in self.analyzer.get_neighbors(key):
            if str(neighbor) not in self.analyzer.copyDict:
                continue
            elif self.analyzer.copyDict[str(neighbor)] == AI_UNKNOWN:
                open_square_number += 1
            elif self.analyzer.copyDict[str(neighbor)] == AI_FLAGGED:
                flags_around += 1
        ...
        for neighbor in self.analyzer.get_cardinal_neighbors(key):
            if str(neighbor) not in self.analyzer.copyDict:
                continue
            if self.analyzer.copyDict[str(neighbor)] in [AI_UNKNOWN, AI_FLAGGED]:
                continue

            adjacent_tile_value = self.analyzer.copyDict[str(neighbor)]
